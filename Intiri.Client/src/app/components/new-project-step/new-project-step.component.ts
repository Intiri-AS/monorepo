...
export class NewProjectStepComponent implements OnInit, OnChanges {
  ...
  ngOnInit() {
    if (
      ...
      this.router.url.includes('customize-moodboard')
    ) {
      //For Client-side moodboard edit
  ...
  ngOnChanges() {
    if (
      this.activatedRoute.snapshot.routeConfig.path === 'new-project' &&
      this.currentStepNo === 1
    ) {
      this.spinner.show();
      this.styleService
        .getStyleImagesByRoom(this.project.room.id)
        .subscribe((styleImages: Array<any>) => {
          this.currentStep.data =
            this.commonUtilsService.shuffleArrayElements(styleImages);
          this.spinner.hide();
        });
    }
    ...
    this.materialProviders = this.commonUtilsService.getUniqueElementsFromArray(
      this.materials.map((e) => e.provider)
    );

    // list providers for products
    this.productProviders = this.commonUtilsService.getUniqueElementsFromArray(
      this.products.map((e) => e.partnerName)
    );
  ...
  getMbFamily(mb) {
    ...
    this.projectService
      .getMbFamily(
        mb.style.id,
        this.project.room.id,
        this.project.colorPalettes
      )
      .subscribe((res: any[]) => {
  ...
  onFileChange(event) {
    if (event.target.files.length > 0) {
      ...
      this.roomSketchImagePaths = Object.keys(event.target.files).map(
        (key, i) =>
  ...
  filterItems() {
    if (this.currentStepNo == 1) {
      if (this.typeFilters.length && this.providerFilters.length) {
        this.currentStep.filteredResult = this.currentStep.data.filter((data) =>
          this.typeFilters.includes(data.materialTypeId.toString())
        );
        this.currentStep.filteredResult =
          this.currentStep.filteredResult.filter((data) =>
