...
export class StyleService {
  ...
  addStyle(styleObj) {
    ...
    formData.append(
      'imageFile',
      styleObj.imageFile,
      `styleImg${styleObj.name.replace(/\s/g, '_')}.png`
    );
  ...
  editStyle(styleId, styleObj) {
    ...
    if (styleObj.imageFile) {
      ...
      return this.http.patch(
        this.apiUrl + 'styles/update/' + styleId,
        formData
      );
    } else {
      return this.http.patch(
  ...
  getStyleImagesByRoom(roomId: number) {
    return this.http.get(
      this.apiUrl + 'StyleImages/getStyleImagesByRoom/' + roomId
    );
  }
  ...
  getStyleImagesForRoomAndStyle(roomId, styleId) {
    return this.http
      .get(
        `${this.apiUrl}styleImages/getStyleImagesByRoomAndStyle/${roomId}/${styleId}`
      )
      .pipe(
        map((styleImages) => {
          if (styleImages) {
  ...
  editStyleImage(styleId, styleObj) {
    ...
    if (styleObj.imageFile) {
      ...
      formData.append(
        'imageFile',
        styleObj.imageFile,
        `styleImg${styleObj.imageFile.name.replace(/\s/g, '_')}.png`
      );
      return this.http.patch(
        this.apiUrl + 'styleImages/update/' + styleId,
        formData
      );
    } else {
      return this.http.patch(
