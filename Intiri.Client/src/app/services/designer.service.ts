...
export class DesignerService {
  ...
  addDesigner(designerObj) {
    return this.http.post(
      `${this.apiUrl}account/register/designer`,
      designerObj
    );
  }
  ...
  editDesigner(designerId, designerObj) {
    ...
    Object.keys(designerObj).forEach((key) =>
      formData.append(key, designerObj[key])
    );
    return this.http.patch(
      this.apiUrl + 'designers/update/' + designerId,
      formData
    );
  }
  ...
  getDesignerClient(consultationPaymentId) {
    return this.http.get(
      this.apiUrl + 'designers/clientConsultation/' + consultationPaymentId
    );
  }
