    ...
    public class UserService : IUserService
    {
        ...
        public async Task<List<string>> DeleteUserRelatedMessagesAsync(int userId)
        {
            List<ChatMessage> chatMessages =
                await _unitOfWork.UserRepository.GetUserMassegesByUserIdAsync(userId);
        ...
        {
            List<string> cloudinaryPublicIds = endUser.Inspirations
                .Select(x => x.PublicId)
                .ToList();

            foreach (Project project in endUser.CreatedProjects)
            {
                cloudinaryPublicIds.AddRange(
                    project.ProjectMoodboards
                        .Where(
                            rs =>
                                rs.RoomDetails != null
                                && !string.IsNullOrEmpty(rs.RoomDetails.SketchPublicId)
                        )
                        .Select(x => x.RoomDetails.SketchPublicId)
                        .ToList()
                );
            }
        ...
        {
            List<string> cloudinaryPublicIds = partner.Products
                .Select(x => x.ImagePublicId)
                .ToList();
