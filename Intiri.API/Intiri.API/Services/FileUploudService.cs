...
namespace Intiri.API.Services
{
    public class FileUploudService : IFileUploudService
    {
        ...
        public FileUploudService(
            ICloudinaryService cloudinaryUploadService,
            ILogger<FileUploudService> logger
        )
        {
        ...
        public async Task<Tuple<HttpStatusCode, string>> TryAddSketchFileAsync(
            RoomDetails roomDetails,
            IFormFile file,
            string fileUploadDestinations
        )
        {
        ...
        public async Task<Tuple<HttpStatusCode, string>> TryAddPartnerLogoFileAsync(
            Partner partner,
            IFormFile file,
            string fileUploadDestinations
        )
        {
            ...
            if (!oldLogoPublicId.IsNullOrEmpty())
            {
                ...
                try
                {
                    deletionResult = await _cloudinaryUploadService.DeleteFileAsync(
                        oldLogoPublicId
                    );
                }
                catch (Exception ex)
        ...
        public async Task<
            Tuple<HttpStatusCode, string, ImageUploadResult>
        > TryAddFileToCloudinaryAsync(
            IFormFile file,
            string fileUploadDestinations,
        ...
        )
        {
            ...
            {
                return new Tuple<HttpStatusCode, string, ImageUploadResult>(
        ...
        public async Task<Tuple<HttpStatusCode, string>> TryDeleteFileFromCloudinaryAsync(
            string oldPublicId
        )
        {
            DeletionResult deletionResult = null;
            ...
            {
                string messageErr =
                    $"Faild delete file from cloudinary services: {deletionResult.Error.Message}";
