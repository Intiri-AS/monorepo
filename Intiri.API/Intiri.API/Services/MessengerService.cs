...
public class MessengerService : IMessengerService
{
    ...
    public async Task<bool> SendMessage(
        ChatMessageInDTO chatMessageInDTO,
        int senderId,
        DateTime sentDate
    )
    {
        if (chatMessageInDTO.Content is null && chatMessageInDTO.Attachments is null)
        {
            throw new ArgumentException(
                $"Both {nameof(chatMessageInDTO.Content)} and {nameof(chatMessageInDTO.Attachments)} are null."
            );
        }
    ...
    public async Task<IEnumerable<ChatPersonOutDTO>> GetChatPersons(User currentUser)
    {
        ...
        if (
            roleNames.Contains(RoleNames.InternalDesigner)
            || roleNames.Contains(RoleNames.ExternalDesigner)
        )
        {
            paymentDataInfos = await GetPaymentDataForUser(
                ProjectToPayerPaymentData(),
                FilterByReceiverId(currentUser.Id)
            );
        }
        else if (
            (
                roleNames.Contains(RoleNames.FreeEndUser)
                || roleNames.Contains(RoleNames.PremiumEndUser)
            )
        )
        {
            paymentDataInfos = await GetPaymentDataForUser(
                ProjectToReceiverPaymentData(),
                FilterByPayerId(currentUser.Id)
            );
        }

        IEnumerable<int> paymentParticipantsIds = paymentDataInfos.Select(pi => pi.ParticipantId);
        IEnumerable<ChatPersonOutDTO> result = await _userRepository.Get(
            ProjectToChatPerson(currentUser),
    ...
    public async Task<IEnumerable<ChatMessageOutDTO>> GetChatHistory(
        int firstUserId,
        int secondUserId
    )
    {
        //TODO: Process what if one of users is deleted
        return await _chatMessageRepository.Get<ChatMessageOutDTO>(
            _mapper,
            message =>
    ...
    private async Task<IEnumerable<PaymentData>> GetPaymentDataForUser(
        Expression<Func<ConsultationPayment, PaymentData>> projection,
        Expression<Func<ConsultationPayment, bool>> filter
    )
    {
        IEnumerable<PaymentData> consultationPayments = await _consultationPaymentRepository.Get(
            projection,
            filter
        );

        return consultationPayments
            .GroupBy(group => group.ParticipantId)
            .Select(
                group =>
                    new PaymentData
                    {
                        ...
                        ExpirationDate = group
                            .OrderByDescending(payment => payment.ExpirationDate)
    ...
    private Expression<Func<ConsultationPayment, PaymentData>> ProjectToPayerPaymentData()
    {
        return payment =>
            new PaymentData
            {
                ParticipantId = payment.PayerId ?? 0,
    ...
    private Expression<Func<ConsultationPayment, PaymentData>> ProjectToReceiverPaymentData()
    {
        return payment =>
            new PaymentData
            {
                ParticipantId = payment.ReceiverId,
                ExpirationDate = payment.ExpirationDate
            };
    }
    ...
    private void SetIsChatExpired(
        IEnumerable<ChatPersonOutDTO> chatPersonOutDTOs,
        IEnumerable<PaymentData> paymentDataInfos
    )
    {
        foreach (var chatPersonOut in chatPersonOutDTOs)
        {
            PaymentData paymentForCurrentPerson = paymentDataInfos.SingleOrDefault(
                pd => chatPersonOut.Id == pd.ParticipantId
            );
            chatPersonOut.ChatPeriodExpired =
                paymentForCurrentPerson != default
                    ? paymentForCurrentPerson.ExpirationDate < DateTime.UtcNow
                    : true;
        }
    }
    ...
    {
        return user =>
            new ChatPersonOutDTO()
            {
                Id = user.Id,
                FirstName = user.FirstName,
                LastName = user.LastName,
                PhotoPath = user.PhotoPath,
                LastSentMessageDate = user.MessagesSent
                    .Where(
                        message => message.RecipientId == currentUser.Id && message.Content != null
                    )
                    .OrderByDescending(message => message.MessageSentDate)
                    .Select(message => message.MessageSentDate)
                    .FirstOrDefault(),
                LastSentMessageContent = user.MessagesSent
                    .Where(
                        message => message.RecipientId == currentUser.Id && message.Content != null
                    )
                    .OrderByDescending(message => message.MessageSentDate)
                    ...
                    .FirstOrDefault(),
                LastReceivedMessageDate = user.MessagesReceived
                    .Where(message => message.SenderId == currentUser.Id && message.Content != null)
                    .OrderByDescending(message => message.MessageSentDate)
                    .Select(message => message.MessageSentDate)
                    .FirstOrDefault(),
                LastReceivedMessageContent = user.MessagesReceived
                    .Where(message => message.SenderId == currentUser.Id && message.Content != null)
                    .OrderByDescending(message => message.MessageSentDate)
                    .Select(message => message.Content)
                    .FirstOrDefault()
    ...
    private PusherMessage CreatePusherMessage(ChatMessage chatMessage)
    {
        return new PusherMessage()
        {
            ...
            MessageAttachments = chatMessage.Attachments.Select(
                a =>
                    new PusherMessageAttachment()
                    {
    ...
    private async Task<ICollection<ChatMessageAttachment>> UploadMessageAttachments(
        IEnumerable<IFormFile> attachments
    )
    {
        ICollection<ChatMessageAttachment> chatMessageAttachments =
        ...
        foreach (IFormFile fileAttachment in attachments)
        {
            try
            {
                ImageUploadResult uploadResult = await _fileUploadService.UploadFileAsync(
                    fileAttachment,
                    ChatMessageNames.AttachmentsFolderName
                );
