...
namespace Intiri.API.DataAccess.Repository
{
    public class StyleRepository : RepositoryBase<Style>, IStyleRepository
    {
        ...
        public StyleRepository(DataContext context, IMapper mapper)
            : base(context)
        {
            _mapper = mapper;
        }
        ...
        {
            return await _context.Styles.Include(s => s.StyleImages).ToListAsync();
        }

        public async Task<Style> GetStyleWithStyleImagesByIdAsync(int styleId)
        {
            return (
                await Get(style => style.Id == styleId, includeProperties: "StyleImages")
            ).SingleOrDefault();
        }
