    ...
    public class ProjectRepository : RepositoryBase<Project>, IProjectRepository
    {
        ...
        public async Task<IEnumerable<Project>> GetProjectsBasicInfoForUser(int userId)
        {
            return await _context.Projects
                ...
                //.Include(p => p.ColorPalettes)
                .Include(p => p.ProjectMoodboards)
                .ThenInclude(p => p.Products.Take(1))
                //.Include(p => p.ProjectMoodboards).ThenInclude(p => p.Style)
                .Include(p => p.ProjectMoodboards)
                .ThenInclude(p => p.StyleImages.Take(2))
                .ToListAsync();
        }

        public async Task<Project> GetLastProjectForUser(int userId)
        {
            IEnumerable<int> result = (await Get(project => project.EndUserId == userId)).Select(
                x => x.Id
            );

            return (
                await Get(
                    project => project.Id == result.Max(),
                    includeProperties: "ColorPalettes,StyleImages,ProjectMoodboards"
                )
            ).FirstOrDefault();
        }
        ...
        {
            return await _context.Projects
                .Include(eu => eu.EndUser)
                .ThenInclude(cp => cp.CreatedProjects)
                .Include(p => p.ProjectMoodboards)
                .ThenInclude(rd => rd.RoomDetails)
