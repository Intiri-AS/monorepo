...
namespace Intiri.API.DataAccess.Repository
{
    public class UserRepository : RepositoryBase<User>, IUserRepository
    {
        ...
        public async Task<int> GetUsersCountAsync<TEntity>()
            where TEntity : class
        {
        ...
        public async Task<IEnumerable<TEntity>> GetUsersAsync<TEntity>()
            where TEntity : class
        {
            //return _context.Set<EndUser>().SingleOrDefault(x => x.Id == id);
        ...
        public async Task<TEntity> GetUserByIdAsync<TEntity>(int id)
            where TEntity : User
        {
            //return _context.Set<EndUser>().SingleOrDefault(x => x.Id == id);
        ...
        public async Task<TEntity> GetUserUsernameIdAsync<TEntity>(string username)
            where TEntity : User
        {
            //return _context.Set<EndUser>().SingleOrDefault(x => x.Id == id);
            return await _context.Users
                .OfType<TEntity>()
                .SingleOrDefaultAsync(eu => eu.UserName == username);
        }
        ...
        public Task<IEnumerable<ClientsPerMonthDTO>> GetClientsPerMonthAsync()
        {
            ...
            IEnumerable<ClientsPerMonthDTO> clientsPerMonth2 = (
                ...
                join clt in _context.Users.OfType<EndUser>()
                    on mnt equals clt.Created.Month
                    into monthGroup
        ...
        public async Task<Designer> GetDesignerByIdWithReviewsAsync(int id)
        {
            ...
            return await _context.Users
                .OfType<Designer>()
                .Include(dr => dr.DesignerRating)
                .Include(c => c.DesignerReviews)
                .ThenInclude(eu => eu.EndUser)
                .Include(m => m.CreatedMoodboards)
                .Include(u => u.Roles)
                .ThenInclude(r => r.Role)
                .SingleOrDefaultAsync(d => d.Id == id);
        ...
        public async Task<Designer> GetDesignerByIdWithClientsAsync(int id)
        {
            ...
            return await _context.Users
                .OfType<Designer>()
                .Include(cp => cp.ConsultationPaymentsReceived)
                .ThenInclude(cl => cl.Payer)
                .Include(cp => cp.ConsultationPaymentsReceived)
                .ThenInclude(cl => cl.Moodboard)
                .ThenInclude(s => s.Style)
                .Include(cp => cp.ConsultationPaymentsReceived)
                .ThenInclude(cl => cl.MoodboardOffer)
                .SingleOrDefaultAsync(d => d.Id == id);
        }
