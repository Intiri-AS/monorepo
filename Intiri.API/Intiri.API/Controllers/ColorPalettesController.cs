...
namespace Intiri.API.Controllers
{
    public class ColorPalettesController : BaseApiController
    {
        ...
        public ColorPalettesController(IUnitOfWork unitOfWork, IMapper mapper)
            : base(unitOfWork)
        {
            _mapper = mapper;
        }
        ...
        {
            IEnumerable<ColorPaletteOutDTO> colorPalettesOut =
                await _unitOfWork.ColorPaletteRepository.GetColorPalettesWithNCSAsync();
            return Ok(colorPalettesOut);
        }

        [Authorize(Policy = PolicyNames.AdminPolicy)]
        [HttpPost("add")]
        public async Task<ActionResult<ColorPaletteOutDTO>> AddColorPalette(
            [FromForm] ColorPaletteInDTO colorPaletteIn
        )
        {
            IEnumerable<ColorPalette> allColorPalettes =
                await _unitOfWork.ColorPaletteRepository.GetColorPalettesAsync();
        ...
        public async Task<ActionResult> UpdateColorPalette(
            int colorPaletteId,
            [FromForm] ColorPaletteInDTO colorPaletteIn
        )
        {
            ColorPalette colorPalette =
                await _unitOfWork.ColorPaletteRepository.GetColorPaletteById(colorPaletteId);
